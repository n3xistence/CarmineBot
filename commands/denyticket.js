...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('denyticket')
        .setDescription('denies the ticket the command is used in.'),
    async execute(interaction, Discord, client, version, helper, db_gen, db_ud) {
        ...
        var vembed = new EmbedBuilder()
            ...
            .setColor('#2f3136')
            .setDescription("Are you sure you want to deny the ticket?")
        ...
        if (!owner) {
            return interaction.reply("There has been an error.")
        }
        ...
        interaction.channel.send({ embeds: [vembed] }).then(msg => {
            ...
            const listener = async (reaction, user) => {
                ...
                if (reaction.emoji.name === "✅" && (reaction.message.id == msg.id) && interaction.user.id === user.id) {
                    owner.send("[❌] Your request has been denied.")
                ...
                if (reaction.emoji.name === "❌" && (reaction.message.id == msg.id) && interaction.user.id === user.id) {
                    var denyembed = new EmbedBuilder()
                        ...
                        .setColor('#FF0000')
                        .setDescription("The ticket has not been denied.")
