...
module.exports = {
    ...
    async execute(interaction, Discord, client, version, helper, db_gen, db_ud) {
        ...
        //universal role
        var role = interaction.guild.roles.cache.find(r => r.id === config.server.roles.guildmember.id)
        member.roles.remove(role);

        //guild specific role
        for (let i = 0;i < config.guilds.length;i++) {
            let role = interaction.guild.roles.cache.find(r => r.id === config.guilds[i].role)
            member.roles.remove(role);
        }

        return interaction.reply({ content: "Successfully unlinked your account.", ephemeral: true })
    }
}