...
module.exports = {
    ...
    async execute(interaction, Discord, client, version, helper, db_gen, db_ud) {
        ...
        if (!/^[0-9]+$/.test(interaction.options.getString("id"))) return interaction.reply({ content: `Please provide a valid Guild ID`, ephemeral: true });
        ...
        if (!/^(<@&)?[0-9]+>?$/.test(guild.role)) return interaction.reply({ content: `Please provide a valid role or role ID`, ephemeral: true });
