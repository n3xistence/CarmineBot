...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('weeklystats')
        .setDescription('shows your weekly stats').addStringOption((option) =>
        option
            .setName("user")
    ...
    async execute(interaction, Discord, client, version, helper, db_gen, db_ud) {
        ...
        let date_weekago = helper.getPastDate(helper.getToday(), 7).replace(/\-/g, "_")
        try {  var data_then = db_ud.prepare(`SELECT * FROM ud${date_weekago} WHERE id=?`).get(link.SMMO_ID) } 
        catch(e) { 
            if (e.message.startsWith("no such table")) return interaction.reply({ content: `No database for \`${date_weekago.replace(/\_/g, "-")}\``, ephemeral: true })
            return interaction.reply({ content: "There has been an issue with the database.", ephemeral: true }) 
        }
        if (!data_then) return interaction.reply({ content: `You have no DB entry for ${date_weekago.replace(/\_/g, "-")}`, ephemeral: true }) 
        ...
        let finalEmbed = new EmbedBuilder()
            ...
            .setDescription(`🗓️ ┊ *__${date_weekago.replace(/\_/g, "-")}__ - __${helper.getToday()}__*\n<:blank:1019977634249187368> ┊ *Updated every 10 minutes.*`)
